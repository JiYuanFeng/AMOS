...
def get_default_configuration(network, task, network_trainer, arch, modality, plans_identifier=default_plans_identifier,
    ...
    temp_dir = network_training_output_dir
    new_training_output_dir = temp_dir.replace('AMOS22', arch)
    dataset_directory = join(preprocessing_output_dir, task)

    if network == '2d':
        plans_file = join(preprocessing_output_dir, task, plans_identifier + "_plans_2D.pkl")
    else:
        if arch == 'swin-unetr' and modality == 'mr':
            plans_file = join(preprocessing_output_dir, task, plans_identifier + "_plans_3D_swinunetr.pkl")
        else:
    ...
    output_folder_name = join(new_training_output_dir, network, task, network_trainer + "__" + plans_identifier)
