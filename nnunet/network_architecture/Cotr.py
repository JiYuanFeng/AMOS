...
from nnunet.network_architecture.DeformableTrans import DeformableTransformer
from nnunet.network_architecture.position_encoding import build_position_encoding
    ...
    def posi_mask(self, x):
        ...
        for lvl, fea in enumerate(x):
            if lvl > 1:
                ...
                masks.append(
                    torch.zeros((fea.shape[0], fea.shape[2], fea.shape[3], fea.shape[4]), dtype=torch.bool).cuda())

        return x_fea, masks, x_posemb

    def forward(self, inputs):
        # # %%%%%%%%%%%%% CoTr
        # inputs -> [1, 1, 64, 160, 160]
        x_convs = self.backbone(inputs)
        # x_conv[0] -> [1, 64, 64, 80, 80]
        # x_conv[1] -> [1, 192, 32, 40, 40]
        # x_conv[2] -> [1, 384, 16, 20, 20]
        # x_conv[3] -> [1, 384, 8, 10, 10]
        x_fea, masks, x_posemb = self.posi_mask(x_convs)
        # x_fea[0] ->[1, 384, 16, 20, 20]
        # x_fea[1] -> [1, 384, 8, 10, 10]
        ...
        x = self.transposeconv_stage2(
            x_trans[:, x_fea[0].shape[-3] * x_fea[0].shape[-2] * x_fea[0].shape[-1]::].transpose(-1, -2).view(
                x_convs[-1].shape))  # x_trans length: 12*24*24+6*12*12=7776
        skip2 = x_trans[:, 0:x_fea[0].shape[-3] * x_fea[0].shape[-2] * x_fea[0].shape[-1]].transpose(-1, -2).view(
            x_convs[-2].shape)
