...
def main():
    ...
    parser.add_argument('-a', '--arch', default='None',
                        help="nnunet, nnformer, swin-unetr, unetr, vnet, cotr")
    ...
    if isinstance(folds, list):
        if (folds[0] == 'all' or folds[0] == 'preset') and len(folds) == 1:
            pass
        else:
    ...
    if model == "3d_cascade_fullres" and lowres_segmentations is None:
        ...
        model_folder_name = join(new_training_output_dir, "3d_lowres", task_name, trainer_class_name + "__" +
                                  args.plans_identifier)
        assert isdir(model_folder_name), "model output folder not found. Expected: %s" % model_folder_name
        lowres_output_folder = join(output_folder, "3d_using model stored in lowres_predictions")
        predict_from_folder(model_folder_name, input_folder, lowres_output_folder, folds, arch, modality, False,
    ...
    model_folder_name = join(new_training_output_dir, model, task_name, trainer + "__" +
                              args.plans_identifier)
    print("using model stored in ", model_folder_name)
    assert isdir(model_folder_name), "model output folder not found. Expected: %s" % model_folder_name
    predict_from_folder(model_folder_name, input_folder, output_folder, folds, arch, modality, save_npz, num_threads_preprocessing,
